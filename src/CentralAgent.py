...
class CentralAgent(object):
    ...
    def _get_potential_actions(self, orders):
        ...
        """
        return {size : [list(action) for action in list(combinations(orders, size))] for size in range(1, self.car_capacity + 1)}
    ...
    def _get_noise(self, variable, is_training):
        """
        Generates noise for the model based on the training status.

        Parameters:
            variable (Variable): The variable to generate noise for.
            is_training (bool): Flag indicating whether the model is in training mode.
